...
const monthsSelected = computed(() => {
  const duration = summaryData.value.duration || '3 Months'
...
const applyDiscount = () => {
  ...
  if (code === 'SUBURBAN10') {
    discountApplied.value = Math.round(subTotalValue.value * 0.1)
    discountFeedback.value = 'Discount applied: 10% off subscription fee.'
  } else {
    discountApplied.value = 0
